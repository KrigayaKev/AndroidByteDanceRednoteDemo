...
public class PublishActivity extends AppCompatActivity implements BasePublishFragment.OnPublishFragmentListener {
  ...
  private static final int VIDEO_USER_ID = 3;
  ...
  private void uploadImagesFirst(String title, String content, List<Uri> imageUris,
    ...
    if (isCustomCover && coverUri != null) {
      ...
      String coverFileName = GiteeUploader.generateCoverFileName(timestamp);
  ...
  private void uploadVideoFile(String title, String content, String coverUrl, Uri videoUri, long duration) {
    GiteeUploader uploader = new GiteeUploader(this);
    long timestamp = System.currentTimeMillis();
    String videoFileName = GiteeUploader.generateVideoFileName(timestamp);
  ...
  private void insertPostWithImages(String title, String content, List<String> imageUrls, String coverUrl) {
    // 使用Repository创建Post对象
    Post post = postRepository.createImagePost(title, content, coverUrl, DEFAULT_USER_ID);

    // 使用Repository创建PostImage列表
    List<PostImage> postImages = postRepository.createPostImages(imageUrls);

    // 使用Repository异步插入数据
    postRepository.insertImagePostAsync(post, postImages, new PostRepository.InsertCallback() {
      @Override
  ...
  private void insertVideoPost(String title, String content, String videoUrl, String coverUrl, long duration) {
    // 使用Repository创建视频Post对象
    Post post = postRepository.createVideoPost(title, content, videoUrl, coverUrl, duration, VIDEO_USER_ID);

    // 使用Repository异步插入数据
    postRepository.insertVideoPostAsync(post, new PostRepository.InsertCallback() {
      @Override
      public void onSuccess(long postId) {
